...
class FieldTest(unittest.TestCase):
    ...
    def test_TextField_loads_file(self):
        field = model.FileTextField(name="f", value="license.LICENSE", present=True)
...
class YamlParseTest(unittest.TestCase):
    ...
    def test_saneyaml_load_can_parse_verbatim_tab_text_unstripped(self):
        ...
        expected = [
            ("single_line", "optional"),
            ("other_field", "value"),
            ("multi_line", "This is a long description\nwith tab.\n"),
        ]
    ...
    def test_saneyaml_dangling_text_is_not_an_invalid_continuation(self):
        ...
        expected = [
            ("single_line", "optional"),
            ("other_field", "value"),
            ("multi_line", "some value and more\ninvalid continuation2"),
        ]
        assert expected == list(result.items())
...
class AboutTest(unittest.TestCase):
    ...
    def test_About_has_errors_with_empty_notice_file_field(self):
        ...
        a = model.About(test_file)
        expected = [Error(INFO, "Field notice_file is present but empty.")]
        result = a.errors
        assert expected == result
...
* Email Test@tester.com for any questions"""
    ...
    def test_About_rejects_non_ascii_names_and_accepts_unicode_values(self):
        ...
        expected = [
            Error(
                WARNING,
                "Field name: ['mat\xedas'] contains illegal name characters (or empty spaces) and is ignored.",
            )
        ]
    ...
    def test_About_boolean_more_than_2_character_value(self):
        test_file = get_test_loc("test_model/parse/boolean_more_than_2_chara_data.about")
        a = model.About(test_file)
    ...
    def test_get_field_names_only_returns_non_empties(self):
        a = model.About()
        a.custom_fields["f"] = model.StringField(name="f", value="1", present=True)
        b = model.About()
        b.custom_fields["g"] = model.StringField(name="g", value="1", present=True)
        abouts = [a, b]
    ...
    def test_get_field_names_does_not_return_duplicates_custom_fields(self):
        a = model.About()
        a.custom_fields["f"] = model.StringField(name="f", value="1", present=True)
        a.custom_fields["cf"] = model.StringField(name="cf", value="1", present=True)
        b = model.About()
        b.custom_fields["g"] = model.StringField(name="g", value="1", present=True)
        b.custom_fields["cf"] = model.StringField(name="cf", value="2", present=True)
        abouts = [a, b]
        # ensure all fields (including custom fields) and
        # about_resource are collected in the correct order
        expected = [
...
"""
        lic_dict = {
            "license1": ["License1", "license1.LICENSE", "", "some_url", "key"],
            "license2": ["License2", "license2.LICENSE", "", "some_url", "key"],
        }
...
class CollectorTest(unittest.TestCase):
    def test_collect_inventory_return_errors(self):
        ...
        err_msg1 = (
            "non-supported_date_format.ABOUT: Field about_resource: Path %s not found" % file_path1
        )
        err_msg2 = (
            "supported_date_format.ABOUT: Field about_resource: Path %s not found" % file_path2
        )
        expected_errors = [
    ...
    def test_collect_inventory_does_not_raise_error_and_maintains_order_on_custom_fields(self):
        ...
        errors, abouts = model.collect_inventory(test_loc)
        expected_errors = [Error(INFO, "Field ['resource', 'custom_mapping'] is a custom field.")]
        assert expected_errors == errors
        expected = ["about_resource: .\nname: test\nresource: .\ncustom_mapping: test\n"]
    ...
    def test_copy_redist_src_no_structure(self):
        ...
        copy_list = [
            get_test_loc("test_model/redistribution/this.c"),
...
class FetchLicenseTest(unittest.TestCase):
    ...
    @mock.patch("attributecode.util.have_network_connection")
    @mock.patch("attributecode.model.valid_api_url")
    def test_pre_process_and_fetch_license_dict_licensedb(
        self, have_network_connection, valid_api_url
    ):
